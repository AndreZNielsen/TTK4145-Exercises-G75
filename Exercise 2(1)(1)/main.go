...
func connectTCP(ip string, port string) {

	input := bufio.NewReader(os.Stdin) // lar bruker skrive melding
	...
	for {
		...
		melding, _ := input.ReadString('\n')
		melding = strings.TrimSpace(melding)
		if melding == "stopp" {//stopper programet 
			fmt.Println("stopper")
			break
		}

		melding = melding + "\000"
		tcp.Write([]byte(melding)) //sender meldingen

		n, _ := tcp.Read(buffer) //leser fra serverne 
		fmt.Printf("serveren sa: %s\n", string(buffer[0:n]))
